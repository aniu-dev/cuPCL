 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloudROR(size_t num_points, float range = 100.0f) {
...
void RadiusOutlierRemoval_test(size_t numPoints, float range)
{
    ...
    cloud = generateRandomCloudROR(numPoints, range);
    ...
    ror.setRadiusSearch(0.5);        // 设置半径为0.1
    ror.setMinNeighborsInRadius(20); // 设置查询点的邻域点个数
    ...
    pcl::cuda::RadiusOutlierRemoval ror_gpu;
    ror_gpu.setInputCloud(&cloud_source);     // 输入点云
    ror_gpu.setRadiusSearch(0.5);        // 设置半径为0.1
    ror_gpu.setMinNeighborsInRadius(20); // 设置查询点的邻域点个数
    ...
    std::cout << "--------------------------------------------------------------------------------------------------\n";
    std::cout << "[PCL] RadiusOutlierRemoval Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] RadiusOutlierRemoval Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";
