 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloud2(size_t num_points, float range = 100.0f) {
...
void getMinMax3D_test(size_t numPoints, float range)
{
    ...
    cloud = generateRandomCloud2(numPoints, range);
    ...
    pcl::cuda::PointXYZ minPt_gpu, maxPt_gpu;
    int test_num = 50;
    double gpu_all_time = 0.0;
    for (int i = 0; i < test_num; i++)
    {
        auto t3 = std::chrono::high_resolution_clock::now();
        pcl::cuda::getMinMax3D(cloud_source, minPt_gpu, maxPt_gpu);
    ...
    std::cout << "[PCL] getMinMax3D Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] getMinMax3D Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";
