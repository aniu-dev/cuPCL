 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloudVoxel(size_t num_points, float range = 100.0f) {
...
void VoxelGrid_test(size_t numPoints, float range)
{
    ...
    cloud = generateRandomCloudVoxel(numPoints, range);
    ...
    vg.setLeafSize(0.5, 0.5, 0.5); // 设置最小体素边长
    ...
    pcl::cuda::VoxelGrid vg_gpu;
    vg_gpu.setInputCloud(&cloud_source);             // 输入点云
    vg_gpu.setLeafSize(0.5, 0.5, 0.5); // 设置最小体素边长
    ...
    std::cout << "--------------------------------------------------------------------------------------------------\n";
    std::cout << "[PCL] VoxelGrid Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] VoxelGrid Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";
