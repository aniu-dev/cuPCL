 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloudLine(size_t num_points, float range = 100.0f) {
...
void projectLine_test(size_t numPoints, float range)
{
    ...
    cloud = generateRandomCloudLine(numPoints, range);
    ...
    line_coeff->values[0] = 0.0;
    line_coeff->values[1] = 0.0;
    line_coeff->values[2] = 0.0;
    line_coeff->values[3] = 0.0;
    line_coeff->values[4] = 0.0;
    line_coeff->values[5] = 1.0;
    //投影滤波器对象
    ...
    plane_coeff_gpu.values.resize(6);
    ...
    proj_gpu.setModelType(pcl::cuda::SACMODEL_LINE);
    ...
    std::cout << "[PCL] projectLine Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] projectLine Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";
