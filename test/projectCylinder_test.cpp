 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloudCylinder(size_t num_points, float range = 100.0f) {
...
void projectCylinder_test(size_t numPoints, float range)
{
    ...
    cloud = generateRandomCloudCylinder(numPoints, range);
    ...
    cylinder_coeff->values[0] = 0.0;
    cylinder_coeff->values[1] = 0.0;
    cylinder_coeff->values[2] = 0.0;
    cylinder_coeff->values[3] = 0.0;
    cylinder_coeff->values[4] = 0.0;
    cylinder_coeff->values[5] = 1.0;
    cylinder_coeff->values[6] = 2.0;
    //投影滤波器对象
    pcl::ProjectInliers<pcl::PointXYZ> proj;
    proj.setModelType(pcl::SACMODEL_CYLINDER);
    proj.setInputCloud(cloud);
    proj.setModelCoefficients(cylinder_coeff);
    ...
    pcl::cuda::ModelCoefficients plane_coeff_gpu;
    plane_coeff_gpu.values.resize(7);
    plane_coeff_gpu.values[0] = 0.0;
    plane_coeff_gpu.values[1] = 0.0;
    plane_coeff_gpu.values[2] = 0.0;
    plane_coeff_gpu.values[3] = 0.0;
    plane_coeff_gpu.values[4] = 0.0;
    plane_coeff_gpu.values[5] = 1.0;
    plane_coeff_gpu.values[6] = 2.0;



    pcl::cuda::ProjectInliersFilter proj_gpu;
    proj_gpu.setInputCloud(&cloud_source);
    proj_gpu.setModelCoefficients(plane_coeff_gpu);
    proj_gpu.setModelType(pcl::cuda::SACMODEL_CYLINDER);
    ...
    std::cout << "[PCL] projectCylinder Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] projectCylinder Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";
