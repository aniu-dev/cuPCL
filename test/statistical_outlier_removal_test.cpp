 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloudSOR(size_t num_points, float range = 100.0f) {
...
void StatisticalOutlierRemoval_test(size_t numPoints, float range)
{
    ...
    cloud = generateRandomCloudSOR(numPoints, range);
    ...
    sor.setMeanK(20);           //统计时查询邻近点个数
    sor.setStddevMulThresh(1.2);  //标准差倍数的阈值，标准差大于该值的就会被去除，一般设置为1
    ...
    pcl::cuda::StatisticalOutlierRemoval sor_gpu;
    sor_gpu.setInputCloud(&cloud_source);     // 输入点云
    sor_gpu.setMeanK(20);           //统计时查询邻近点个数
    sor_gpu.setStddevMulThresh(1.2);  //标准差倍数的阈值，标准差大于该值的就会被去除，一般设置为1
    ...
    std::cout << " PCL Point NUms:" << cloud_out->points.size() << std::endl;
    std::cout << " CUPCL Point NUms:" << cloud_out_gpu.size() << std::endl;

    std::cout << "--------------------------------------------------------------------------------------------------\n";
    std::cout << "[PCL] StatisticalOutlierRemoval Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] StatisticalOutlierRemoval Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";
