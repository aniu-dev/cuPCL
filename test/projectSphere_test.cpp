 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloudSphere(size_t num_points, float range = 100.0f) {
...
void projectSphere_test(size_t numPoints, float range)
{
    ...
    cloud = generateRandomCloudSphere(numPoints, range);
    ...
    sphere_coeff->values[3] = 2.0;
    pcl::ProjectInliers<pcl::PointXYZ> proj;
    proj.setModelType(pcl::SACMODEL_SPHERE);
    proj.setInputCloud(cloud);
    proj.setModelCoefficients(sphere_coeff);
    ...
    plane_coeff_gpu.values.resize(4);
    plane_coeff_gpu.values[0] = plane_coeff_gpu.values[2] = 0;
    plane_coeff_gpu.values[1] = 0.0;
    plane_coeff_gpu.values[3] = 2.0;

    pcl::cuda::ProjectInliersFilter proj_gpu;
    proj_gpu.setInputCloud(&cloud_source);
    proj_gpu.setModelCoefficients(plane_coeff_gpu);
    proj_gpu.setModelType(pcl::cuda::SACMODEL_SPHERE);
    ...
    std::cout << "[PCL] projectSphere Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] projectSphere Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";
