 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRandomCloud(size_t num_points, float range = 100.0f) {
...
void compute3DCentroid_test(size_t numPoints,float range)
{
    ...
    cloud = generateRandomCloud(numPoints, range);
    ...
    for (int i = 0; i < test_num; i++)
    {
        ...
        pcl::cuda::compute3DCentroid(cloud_source, centroid_gpu);
    ...
    std::cout << "[PCL] compute3DCentroid Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] compute3DCentroid Time: " << gpu_all_time / (test_num -1) << " ms " << "\n";
