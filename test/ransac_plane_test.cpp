 ...
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr generateRansacPlaneData(size_t num_points) {
...
void ransacPlane_test(size_t numPoints)
{
    ...
    cloud = generateRansacPlaneData(numPoints);
    ...
    seg.setModelType(pcl::SACMODEL_PLANE);   //设置拟合模型参数
    ...
    ransac.setModelType(pcl::cuda::SACMODEL_PLANE);
    ...
    std::cout << "[PCL] ransacPlane Time: " << pcl_time << " ms " << "\n";
    std::cout << "[GPU] ransacPlane Time: " << gpu_all_time / (test_num - 1) << " ms " << "\n";

    //------------------模型系数---------------------
    cout << "PCL 拟合平面的模型系数为：" << endl;
